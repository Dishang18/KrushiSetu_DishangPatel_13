...
export const getALLProducts = async (req, res) => {
  try {
    console.log("Fetching certified farmers with products");
    
    // Find all farmer documents with 'certified' verification status
    const certifiedFarmerDocs = await FarmerDocument.find({ 
      verificationStatus: "certified" 
    });
    
    // Extract farmer IDs
    const certifiedFarmerIds = certifiedFarmerDocs.map(doc => doc.farmerId);
    console.log("Certified farmer IDs:", certifiedFarmerIds);
    
    // Find all users with those IDs who are farmers
    const certifiedFarmers = await User.find({ 
      _id: { $in: certifiedFarmerIds },
      role: "farmer" 
    }).select("_id name");

    console.log("Certified farmers:", certifiedFarmers);
    
    // Get product counts for each certified farmer
    const farmersWithProductCounts = await Promise.all(
      certifiedFarmers.map(async (farmer) => {
    ...
    console.error("Error fetching certified farmers with product counts:", error);
    res.status(500).json({
      success: false,
      message: "Failed to fetch certified farmers with product counts",
      error: error.message,
    });
  }
};
...
export const getFarmerProducts = async (req, res) => {
  try {
    ...
    const products = await Product.find({ farmer_id: id }).sort({
      createdAt: -1,
    });
  ...
  try {
    ...
    if (req.body.traceability && typeof req.body.traceability === "string") {
      ...
      updateData.traceability = {
        harvest_date:
          traceabilityFields.harvest_date || req.body.harvest_date || "",
        harvest_method:
          traceabilityFields.harvest_method || req.body.harvest_method || "",
        certified_by:
          traceabilityFields.certified_by || req.body.certified_by || "",
      };
    ...
    if (updateData.available_quantity !== undefined) {
      ...
      if (
        isNaN(updateData.available_quantity) ||
