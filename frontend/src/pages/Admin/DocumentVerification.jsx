...
function DocumentVerification() {
  ...
  useEffect(() => {
    const fetchPendingDocuments = async () => {
      try {
        ...
        const response = await axiosInstance.get('/documents/pending/all');
  ...
  const handleDownloadDocument = async (docId) => {
    try {
      window.open(`http://localhost:5000/api/documents/${docId}?token=${localStorage.getItem("token")}`, '_blank');
    } catch (err) {
  ...
  const handleVerifyDocuments = async () => {
    ...
    try {
      ...
      // Use the verify-all endpoint to verify both documents at once
      const response = await axiosInstance.put(
        `/documents/verify-all/${selectedFarmer.farmerId}`,
        {
          status: verificationStatus,
          remarks: verificationRemarks,
          farmerName: selectedFarmer.farmerName,
          farmerMongoId: selectedFarmer.farmerId // Send MongoDB _id to the backend
        }
      );
      ...
      if (response.data) {
        setSuccessMessage(
          verificationStatus === 'verified'
            ? response.data.certificateIssued
